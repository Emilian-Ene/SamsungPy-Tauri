    ...
    async with MDC(f"{ip}:{port}") as mdc:
        ...
        if action == "cli_get":
            ...
            if command_name == "timer_15":
                ...
                result = await mdc.timer_15(display_id, timer_id, ())
        ...
        if action == "cli_set":
            ...
            if command_name == "timer_15":
                ...
                if not timer_data:
                    raise ValueError("timer_15 SET requires timer values after timer_id")
                result = await mdc.timer_15(display_id, timer_id, timer_data)
                return {
